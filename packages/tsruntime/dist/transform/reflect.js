...
function getReflect(ctx) {
    ...
    function serializeReference(type) {
        ...
        if (typeArgs !== undefined) {
            allTypes = typeArgs.map(function (t) { return reflectType(t); });
        }
    ...
    function serializePropertySymbol(sym) {
        try {
            ...
            if (!serializedType || typeof serializedType.kind === 'undefined') {
                var fallbackType = { kind: types_1.TypeKind.Unknown2 };
                return {
                    name: name_1,
                    modifiers: modifiers,
                    type: __assign({}, fallbackType)
                };
            }
            ...
            // console.warn('serializePropertySymbol failed for symbol:', (sym as any).escapedName || sym.name, error.message);
